...
export function CommunityCreatePostModal({
  ...
  function validatePost() {
    ...
    if (contentType === "general" && !content && selectedImageUris.length === 0)
      return "Escreva uma descrição ou adicione uma imagem.";
    if (contentType === "sos" && !content && selectedImageUris.length === 0)
    ...
    if (contentType === "electric" && !content && selectedImageUris.length === 0)
  ...
  async function handleCreatePost() {
    try {
      ...
      setSavingPost(true);
      const uploadedImages = selectedImageUris.length
        ? await Promise.all(selectedImageUris.map(uploadImage))
        : [];
